...
| 调优 | `sp-tuning`, `dp-tuning`, `longitudinal-tuning`, `sp-brand-*` | `list_sp_settings`, 预设, `diff_params` |
| Engage 分诊 | `engage-troubleshooting` + `builtin_engage_triage` | Panda → SecOC → 指纹 → dashcam → LKAS |
| C3 Panda | `c3-dos-panda` + `PANDA_FLASH.md` | F4/H7 固件、多 Panda（F4+H7）、统一 `pandad` |
| GitHub Runner | `github-runner` + `GITHUB_RUNNER.md` | C3 prebuilt CI、自建 Runner |
...
技能文件 `ai/skills/sp-tuning/`、`sunnypilot-settings/`、`carrot-legacy/`（CP 用户可选）、`vehicle-adaptation/`、`c3-dos-panda/`、`github-runner/` 已整理。内置知识库 `rag_seed` 含 CP→DP 横向/纵向/UI/适配摘要。
...
- **~210+ LLM tools**（含 Git push、OTA、Panda 刷机、GitHub Runner、批量路线评分、pytest/scons、SSH 只读、Comma 云路线等）
- **48 Skills**（registry v24；`requires_tools` 运行时过滤；含 `sp-brand-*`、`c3-dos-panda`、`github-runner`）
...
| `release/ci/install_github_runner.sh` | `github_runner_status`, `github_runner_recovery_hint`, `install_github_runner` | C3 自建 Runner + prebuilt；见 `ai/docs/GITHUB_RUNNER.md` |
