...
def _to_thread(fn, *args, **kwargs):
  ...
  Scheduler actions are mostly blocking I/O; keep them from stalling the
  async runtime by dispatching to the default executor.
  """
...
async def device_wifi_connected() -> bool:
  try:
    out = await run_command("ip_addr")
...
async def scheduler_execute_action(action: str, _payload: dict[str, Any]) -> str:
  if action == "read_last_log":
    ...
    res = await _to_thread(read_manager_log, _PARAMS, lines=80)
    text = res.get("log", "") or ""
    src = res.get("source", "")
    return (f"[{src}] " if src else "") + text[:400] or "empty"
  if action == "read_usage":
    u = await _to_thread(load_usage, _PARAMS)
  ...
  if action == "read_tune_snapshot":
    ...
    snap = await _to_thread(list_sp_settings, _PARAMS, brand=state.brand)
    return f"{snap.get('setting_count', 0)} settings"
  if action == "memory_ping":
    m = await _to_thread(get_memory, _PARAMS)
  ...
  if action == "snapshot_tune":
    ...
    state = get_state_reader().update(timeout=0)
    await _to_thread(save_tune_snapshot, _PARAMS, label="scheduler", brand=state.brand or "")
    snap = await _to_thread(snapshot_tune_state, _PARAMS, brand=state.brand)
    return f"params={snap.get('param_count', 0)}"
  if action == "trip_review_offroad":
    from ai.tools.diagnostics_tools import trip_review
    state = get_state_reader().update(timeout=0)
    review = await _to_thread(trip_review, _PARAMS, get_state_reader, brand=state.brand or "")
    summary = "; ".join(review.get("recommendations") or [])[:300]
    await _to_thread(append_note, _PARAMS, f"[offroad] {summary}", tags=["auto", "trip_review"])
  ...
  if action == "check_critical_events":
    ...
    ev = await _to_thread(read_onroad_events, get_state_reader)
  ...
  if action == "post_drive_review_offroad":
    ...
    state = get_state_reader().update(timeout=0)
    built = await _to_thread(build_post_drive_summary, _PARAMS, get_state_reader, brand=state.brand or "")
    await _to_thread(append_note, _PARAMS, f"[offroad] {built.get('text', '')[:300]}", tags=["auto", "voice_summary"])
  ...
  if action == "check_param_watchlist_offroad":
    ...
    res = await _to_thread(check_param_watchlist, _PARAMS)
    if res.get("drifted"):
  ...
  if action == "git_fetch_wifi":
    ...
    res = await _to_thread(git_fetch)
  ...
  if action == "chat_notify":
    ...
    await _to_thread(append_note, _PARAMS, f"[scheduler] {summary}", tags=["auto", "chat_notify"])
    return summary
